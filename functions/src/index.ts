...
- rationale: 추천 이유 (1-2문장)
  * 반드시 사용자 입력 "${mood}${noteSection}"를 직접 언급
  * "~라고 나누셨기에" 또는 "~한 마음에" 형식으로 시작
  * 사용자 입력과 추천 구절의 연결점 명확히 제시`
        : `[추천 작업]
사용자: ${userLabel}
입력: "${mood}${noteSection}"
...
- rationale: 이 말씀이 주어진 이유 (1~2문장)
  * 사용자가 입력한 "${mood}${noteSection}"와 직접 연결
  * 사용자가 특정 구절을 언급했다면 그 구절을 선택한 마음을 반영
  * 사용자가 감정/상황을 공유했다면 그 맥락에 맞는 이유 제시
  * 예: "${userLabel}께서 '${mood}'라고 나누신 마음에 이 말씀이 응답이 되기를 바랍니다."
...
- 사용자 입력을 직접 언급하며 시작
- 사용자가 구절 입력 시: "~라고 나누신 말씀에 대한 깊은 묵상이..."
- 사용자가 감정 입력 시: "~한 마음을 나누셨기에..."
- 일반적인 위로보다 구체적인 연결점 제시

6) 예시
korean: "하나님은 세상을 극진히 사랑하셔서 독생자를 내어주셨습니다. 이는 인류를 향한 하나님의 무조건적인 사랑과 구원 계획을 보여줍니다. 믿는 자마다 영생을 얻는다는 이 약속은 오늘 우리에게도 변함없는 소망입니다."
rationale: "${userLabel}께서 나누신 '${mood}'에 이 말씀이 정확한 답이 되리라 믿습니다. [구체적인 연결점]"
