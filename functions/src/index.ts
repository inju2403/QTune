...
export const recommendVerse = functions.https.onCall(
  async (data: RecommendVerseRequest, _context) => {
    try {
      ...
      logger.info("recommendVerse called", { locale, mood, note });
...
export const generateKoreanExplanation = functions.https.onCall(
  async (data: GenerateKoreanExplanationRequest, _context) => {
    try {
      ...
      logger.info("generateKoreanExplanation called", { verseRef, mood });
